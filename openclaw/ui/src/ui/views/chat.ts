...
function renderWelcomeState(props: ChatProps): TemplateResult {
  ...
  return html`
    <div class="agent-chat__welcome" style="--agent-color: var(--accent)">
      ...
      ${
        avatar
          ? html`<img src=${avatar} alt=${name} style="width:64px; height:64px; border-radius:20px; object-fit:cover;" />`
          : nothing
      }
      <div class="agent-chat__eyebrow">LOBSTER MATRIX CONTROL</div>
      <div class="agent-chat__hero">
        ${
          avatar
            ? nothing
            : html`<div class="agent-chat__avatar agent-chat__avatar--logo"><img src=${logoUrl} alt="OpenClaw" /></div>`
        }
        <div class="agent-chat__title-block">
          <h1>龍蝦母體控制台</h1>
          <h2>主控節點已接管，${name} 隨時可介入目前會話與系統狀態。</h2>
        </div>
      </div>
      <div class="agent-chat__badges">
        <span class="agent-chat__badge"><img src=${logoUrl} alt="" /> Control surface online</span>
        <span class="agent-chat__badge">Session routing retained</span>
        <span class="agent-chat__badge">Tool stream unchanged</span>
      </div>
      <p class="agent-chat__lead">
        這裡是母體層級的操作入口，用最少步驟接管對話、檢視狀態、下達下一步控制命令。
      </p>
      <p class="agent-chat__hint">直接輸入訊息開始控制，或使用下列快捷操作；按 <kbd>/</kbd> 可叫出命令。</p>
      <div class="agent-chat__suggestions">
        ${WELCOME_SUGGESTIONS.map(
          (text) => html`
            ...
            >
              <span class="agent-chat__suggestion-label">快速操作</span>
              <span class="agent-chat__suggestion-text">${text}</span>
            </button>
          `,
        )}
      </div>
